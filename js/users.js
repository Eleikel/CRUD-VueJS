window.addEventListener("load", () => {
  const app = Vue.createApp({
    ...
    mounted() {
      this.$refs.name.focus(); // Poniendole focus al input de Name
    },
    ...
    methods: {
      ...
      processUser(event) {
        ...
        if (this.operation === "Register") {
          this.user.id = this.findMaxId() + 1; // Id del usuario que se registre
          ...
          const update = this.users[this.userIndex];
